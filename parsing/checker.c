...
/*   Updated: 2024/01/23 15:13:19 by vilibert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_syntax_error2(t_data *data, char c)
{
	if (!c)
...
int	check_syntax_error(t_data *data, char *str)
{
	...
	while (str[i])
	{
		...
		else if ((!cmd && str[i] == '|') || str[i] == '<' || str[i] == '>' || !str[i])
			return (print_syntax_error2(data, str[i]), 1);
		if (str[i])
			i++;
		// printf("4 : %c\n", str[i]);
	}
